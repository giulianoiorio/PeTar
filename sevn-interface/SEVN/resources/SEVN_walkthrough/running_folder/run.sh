...
#SEVNPATH
#-------------------------------
SEVN="../../../" #Complete path to the SEVN folder
...
#-------------------------------
if [ "$cmdargument" = "" ]; then
  LISTBIN="imf2.3_n1e4.dat" #Complete path to input file (list of binaries or single stars)
else
  LISTBIN=$cmdargument
fi
...
#METALLICITY
#-------------------------------
Z="0.0002"  #Stellar metallicity - [*list][number]. If list use the Z in the input file otherwise overwrite all the Zs.
...
CELAM="-1" #if >0 Constant Lambda in binding energy (Eq. 69 in Hurley02). If -11 use Lambda from MOBSE (other options available).
