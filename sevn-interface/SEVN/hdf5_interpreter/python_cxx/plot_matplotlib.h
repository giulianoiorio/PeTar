...
template <typename T> const std::string n2s(T val) {
    ...
    if (stream.fail())
        std::cout<<"Cannot convert into a string"<<std::endl;

    return stream.str();
}

#define _to_string(A) n2s(A)
...
private:
    ...
    bool _autorange;
    std::string _pythoncmd, xpoints;
    static int counter_mrk, counter_clr;
    double _range_x1, _range_x2, _range_y1, _range_y2;  // Range of the plot

    const std::string get_next_marker(){

        int pos = counter_mrk%marker.size();
        counter_mrk++;
        return marker[pos];

    }

    const std::string get_next_color(){

        int pos = counter_clr%color.size();
        counter_clr++;
        return color[pos];
...
public:
    ...
    const std::string get_marker(){
        return ("'" + get_next_marker() + get_next_color() + "'");
    }
    ...
    {
        _autorange=false;
        this->PyRun_SimpleStringStd("pltt.axis(["+_to_string(x1)+", "+_to_string(x2)+", "+_to_string(y1)+", "+_to_string(y2)+"])");
    }
    ...
    {
        this->PyRun_SimpleStringStd("pltt.figure("+_to_string(fignumber)+")");
    }
    //! \brief Set a subplot number
    void subplot(int plotnumber)
    {
        this->PyRun_SimpleStringStd("pltt.subplot("+_to_string(plotnumber)+")");
    }
    ...
    void add_somedata(const std::vector<double> * X, const std::vector<double> * Y, std::string properties="'o'")
    {
        ...
        for(int i=0;i<X->size();i++)
        {
            // Add points
            if(i>0) { xpoints += ","; }
            xpoints += _to_string(X->at(i));
            if(i>0) { ypoints += ","; }
            ypoints += _to_string(Y->at(i));
    ...
    void add_somedata(const std::vector<double> * Y, std::string properties="'o'")
    {
        // Plot Points:
        std::string ypoints = "";
...
int plot_matplotlib::counter_mrk = 0;
int plot_matplotlib::counter_clr = 0;
