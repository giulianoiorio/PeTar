...
namespace utilities{
    ...
    constexpr double G = 3.92934097327e8; //RSUN^3 YR^-2 MSUN^-1
    constexpr double yr_cgs = 3.1557600e7; //yr in s
    constexpr double Rsun_cgs = 6.95700e10; //rsun in cm
    constexpr double Msun_cgs = 1.98892e33; //msun in g
    constexpr double G_cgs = G*Rsun_cgs*Rsun_cgs*Rsun_cgs/(Msun_cgs*yr_cgs*yr_cgs); //cm^3 s^-2 g^-1


    constexpr double Sigma_StefBoltz = 7.144796315707217e-17; //LSun^3 RSun^-2 K^-4
    constexpr double Myr_to_yr = 1.0e6;
    constexpr double yr_to_Myr = 1.0e-6;
    constexpr double AU_to_RSun = 2.150939909288282e2;
    constexpr double kms_to_RSunyr = 4.5373006461e1;
    constexpr double LSun_to_Solar = 1.25397997879e1;
    constexpr double c = 1.36024851337e7; // RSun/yr
    constexpr double km_to_RSun = 1.4378145219e-6;
    constexpr double g_to_MSun = 5.02785431289e-34;
     ...
     * @return return the Eddington accretion rante in Msun/yr
     */
    double dMdt_Eddington_accretion(Star *donor, Star *accretor);
    ...
    std::string common_log_print(const std::string& label, System* system, ListP... args){
        ...
        std::string id_str=std::to_string(get_ID(system)),time=std::to_string(get_current_time(system));
  ...
  */
  inline std::string gen_filename(const std::string &_folder, const std::string &_fname, bool print_threads=true){
      ...
      if (print_threads){
          ...
          return_string = folder + "/" + _fname.substr(0, found_extension) + "_" + std::to_string(omp_get_thread_num()) + _fname.substr(found_extension);
      } else {
