...
public:
    ///MEMBERS///
    bool circularised; /*!< If true the binary has been circularised.
 * NB this is set to true only when some process circularises the binary, the it is set to false again.
 * Notice that the flag circularised=true should be set only when all the other ecc variation can be neglected*/
    ...
    bool disable_OmegaSPin_check; /*!< If true, the OmegaSpin (from binary evolution) in adpative timestep is disabled */
    ...
    Binstar(IO *_io, std::vector<std::string> &params, size_t &_ID, unsigned long _rseed=0){
        ...
        ************************************/
        ...
        circularised=broken = onesurvived=empty=false;
        ...
        disable_e_check = disable_a_check = disable_DM_check = disable_OmegaRem_NS_check = disable_OmegaSPin_check =force_tiny_dt = false;
        rseed = _rseed;
        /************************************/
    ...
    ///METHODS///
    inline double getp(const size_t &id) const {return property[id]->get();}
    inline double getp_0(const size_t &id) const {return property[id]->get_0();}
    ...
    virtual void evolve() {
        ...
        *****************************************************************/
        ...
        for (int i=0; i<2; i++){
            ...
            if (get_svpar_bool("ev_set_maxCO") and std::isnan(this_star->get_MCO_max()) and this_star->getp(Phase::ID)>=TerminalHecoreBurning and !this_star->amiremnant()){
                this_star->set_MCO_max();
            }
            if (get_svpar_bool("ev_set_minHE") and std::isnan(this_star->get_MHE_min()) and this_star->getp(Phase::ID)>=TerminalHecoreBurning and !this_star->amiremnant()){
    ...
    inline bool breaktrigger() const {
        ...
        //When you have to check for Worldtime larger than tf check also if the Worldtime are sync //TODO create a BinTime?
        //else if(!break_at_remnant && are_Worldtime_sync()) return star[0]->getp(Worldtime::ID) >= tf; //TODO Just for debugging purpose!
        else if(!break_at_remnant){
            return star[0]->getp(Worldtime::ID) >= tf; //TODO Just for debugging purpose!
        }
        else{
    ...
    inline bool isoutputtime() {
        //Use what is already implemented in star
        if (star[0]->printevents())
...
private:
    ...
    Star *star[2]; /*!< Pointers to the stars in the binary system*/
